 ...
 *
 * This is a pure-Java decoder used by dicttool to read a static (VERSION2xx) dictionary
 * off-device, without relying on the native library.
 */
@UsedForTesting
public class Ver2DictDecoder extends AbstractDictDecoder {
    ...
    static class PtNodeReader {
        ...
        static int readChildrenAddress(final DictBuffer dictBuffer, final int ptNodeFlags) {
    ...
    public DictionaryHeader readHeader() throws IOException, UnsupportedFormatException {
        ...
        final int version = BinaryDictDecoderUtils.checkFormatVersion(mDictBuffer);
        // TODO: Remove this field.
        mDictBuffer.readUnsignedShort(); // Option flags.
        final int headerSize = mDictBuffer.readInt();
        if (headerSize < 0) {
            throw new UnsupportedFormatException("header size can't be negative.");
        }
        final HashMap<String, String> attributes = new HashMap<>();
        while (mDictBuffer.position() < headerSize) {
            // CharEncoding.readString always advances the position, so this loop terminates.
        ...
        final DictionaryHeader header = new DictionaryHeader(headerSize,
                new FormatSpec.DictionaryOptions(attributes),
